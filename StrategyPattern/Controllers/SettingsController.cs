...
namespace StrategyPattern.Controllers
{
    ...
    public class SettingsController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            if(User.Claims.Where(x=>x.Type == Settings.CalimDatabaseType).FirstOrDefault() != null)
            {
                settings.DatabaseType = (DatabaseType)int.Parse(User.Claims.First(x => x.Type == Settings.CalimDatabaseType).Value);
            }
            else
            {
        ...
        public async Task<IActionResult> ChangeDb(int databaseType)
        {
            ...
            var newClaim = new Claim(Settings.CalimDatabaseType, databaseType.ToString());
            var claims = await _userManager.GetClaimsAsync(user);

            var hasDatabaseTypeClaim = claims.FirstOrDefault(x => x.Type == Settings.CalimDatabaseType);
